function [ERRORS] = kalman_error_progression_v2(warmup, test, A, C, Q, R, init_x, init_V, type)
...
progs_col = find(strcmp(test(1,:), 'FirstProg') == 1, 1); 

data = [test(2:end,id_col), test(2:end,readings_col), test(2:end, progs_col)]; %take just the IDs and readings
...
for n = 1:N
    y = data{n,2}; %get data
    p = data{n,3}; %get progression date   
    ...
    error{1,2} = 'Visits before progression (6 mos)';
    error{1,3} = 'Errors';
    error{1,4} = 'First Visit';
    step = 2; %start from second row
    if p > warmup
        for t = warmup:p %number of observations
            for j = 0:p-t-1
                x = [y(:,j+1:(j+t)), NaN([num_measurements, p - (t+j)])];%predict all visits from t until the end of the horizon
                [z,~,~,~]=kalman_filter(x,A,C,Q,R,init_x,init_V);
                %for s = j+t+1:p %months ahead
                    err_vec = -y(:,p) + z(:,p-j); %predicted - observed
                    error{step, 1} = t; %record number of observations
                    error{step, 2} = p-(j+t); %record number of months before progression
