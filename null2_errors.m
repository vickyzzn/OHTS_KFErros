function [ERRORS] = null2_errors(warmup, test, type)
        ...
        for t = warmup:T %number of observations
            for j = 0:T-t-1 %first visit - 1
                ...
                for r = 1:rows
                    if r == 1 %MD
                        z(r,1:(j+t)) = 0; 
                        curr = y(r,t+j); %current value of MD
                        for k = j+t+1:T
                            z(r,k) = curr - (k-(j+t))*0.25; %assume a drop of 0.25 dB per 6 month period
                        end
                    elseif r == 3 %PSD
                        z(r,1:(j+t)) = 0;
                        curr = y(r, t+j); %current PSD
                        for k =j+t+1:T
                            z(r,k) = curr + (k-(j+t))*0.5; %assume an increase of 0.5 dB per 6 month period
                        end
                        
                    else
                        avg(r) = mean(y(r,j+1:(j+t)));
                        z(r,:) = z(r,:)*y(r, t+j);   %assume no change in IOP
                    end
                    
                    
                end
                for s = j+t+1:T %months ahead
                    err_vec = -y(:,s) + z(:,s); %predicted - observed
