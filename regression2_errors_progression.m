function [ERRORS] = regression2_errors_progression_v2(warmup, test, type)
        ...
        for t = warmup:p %number of observations
            for j = 0:p-t-1
                ...
                for r = 1:rows
                    ...
                    pred_x = pred_x - p;
                    betas(2) = y(r,j+t);
                    z(r,j+t+1:p) = polyval(betas, pred_x);
                end
                %for s = j+t+1:p %months ahead
                    err_vec = -y(:,p) + z(:,p); %predicted - observed
                    error{step, 1} = t; %record number of observations
                    error{step, 2} = p-(j+t); %record number of visits before progression
